...
public class MergeSorter_IMPORTANTE {
		...
		static void mergesort(int[] a, int min, int max) {
			...
			// ordino la seconda met�
			mergesort(a, med+1, max);
			// merge delle due parti (fusione)
			merge(a, min, med, max);
		}
		...
		// risultato deve fare il merge in a delle due parti in modo che siano ordinati
		private static void merge(int[] a, int min, int med, int max) {	
			...
			int sx= min;
			int dx= med+1; //IMPORTANTE
			int temp[] = new int[max-min+1]; //IMPORTANTE
